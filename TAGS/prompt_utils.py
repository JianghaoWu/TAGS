...
def get_joint_domain_prompt(question, options, num_fields=3):
    domain_format = "Medical Field: " + " | ".join([f"Field{i}" for i in range(num_fields)])
    classifier_instruction = (
        "You are a senior medical expert tasked with classifying clinical multiple-choice problems "
        "into the most relevant areas of medical science.\n"
        "Your role is strictly to determine and output the classification.\n"
        "**Do not provide any explanation, reasoning, or commentary. Only output the final classification.**"
    )

    prompt = (
        "Please complete the following task:\n"
        f"1. Carefully analyze the following medical question:\n'''{question}'''\n"
        f"2. The corresponding options are:\n'''{options}'''\n"
        f"3. Based on both the question and the options, determine the top {num_fields} most relevant subfields of medicine that are required to solve this question.\n"
        f"4. You must only output in the exact format:\n'''{domain_format}'''\n"
    ...
    return classifier_instruction, prompt



def get_question_domains_prompt(question, NUM_QD_=NUM_QD):
    question_domain_format = "Medical Field: " + " | ".join(["Field" + str(i) for i in range(NUM_QD_)])
